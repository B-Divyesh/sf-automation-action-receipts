...
`site/public/terminal-demo.svg` is a self-hosted capture generated by
`scripts/capture-terminal-demo.mjs` from an actual released
`action-receipts demo` process. The unedited terminal text is retained at
`artwork/source/terminal-demo-capture.txt`; the SVG wraps those exact lines for
display. Its cursor is static, so the asset has no ambient or reduced-motion
animation. Captured 2026-08-29 from the project binary; distributed under MIT.

`site/public/apple-touch-icon.png` is a 180 × 180 raster rendering of the
project's evidence-chain SVG favicon. It uses the same paper, ink, signal, and
proof colors and introduces no new mark. Rendered locally with ImageMagick on
2026-08-29 from the project-owned SVG; distributed under MIT.
