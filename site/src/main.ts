...
const DEMO = location.pathname.startsWith('/demo') || new URLSearchParams(location.search).get('demo') === '1';
const store = (key: string) => `${DEMO ? 'demo:' : ''}${key}`;
const byId = <T extends HTMLElement>(id: string) => document.getElementById(id) as T | null;

if (DEMO && !location.pathname.startsWith('/demo')) location.replace('/demo/');

function setupTheme() {
  const button = byId<HTMLButtonElement>('theme-toggle'); if (!button) return;
  const saved = localStorage.getItem(store('ar_theme'));
  if (saved === 'dark' || saved === 'light') document.documentElement.dataset.theme = saved;
  const paint = () => { const dark = document.documentElement.dataset.theme === 'dark' || (!document.documentElement.dataset.theme && matchMedia('(prefers-color-scheme: dark)').matches); button.textContent = dark ? 'Use light theme' : 'Use dark theme'; };
